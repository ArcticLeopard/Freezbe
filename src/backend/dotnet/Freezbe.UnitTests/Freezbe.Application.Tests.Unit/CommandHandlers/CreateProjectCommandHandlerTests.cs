...
public class CreateProjectCommandHandlerTests
{
    ...
    public async Task HandleAsync_CommandWithExistingsSpaceId_ShouldSuccessfullyAddsProject()
    {
        ...
        var handler = new CreateProjectCommandHandler(_fakeTimeProvider, spaceRepositoryMock.Object);
        var command = new CreateProjectCommand(Guid.NewGuid(), "Test description", existingsSpaceId);
    ...
    public async Task HandleAsync_CommandWithNotExistingsSpaceId_ShouldThrowSpaceNotFoundException()
    {
        ...
        spaceRepositoryMock.Setup(p => p.GetAsync(existingsSpaceId)).ReturnsAsync(new Space(Guid.NewGuid(),"Description", _fakeTimeProvider.GetUtcNow()));
        var handler = new CreateProjectCommandHandler(_fakeTimeProvider, spaceRepositoryMock.Object);
        var command = new CreateProjectCommand(Guid.NewGuid(), "Test description", Guid.NewGuid());
